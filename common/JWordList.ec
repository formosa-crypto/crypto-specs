...
(*from JazzCommon*) require import EclibExtra.
require BitEncoding.
...
by rewrite b2i1 chunkpadE // C /= size_nseq /#.
qed.
...
rewrite chunkify_cat ?size_take 1..3:/# chunkify_chunk // ?size_take' 1..2:/# chunk1 1:/#. 
...
lemma map_chunkifyK ['a] d (f:'a list -> 'a list) n bs:
 0 < n =>
 (forall l, size l<=n => f l = chunkfill d n l) =>
 flatten (map f (chunkify n bs))
 = chunkfill d n bs. (* bs ++ chunkify_fillsuffix d n (size bs).*)
proof.
...
admitted (*
rewrite chunkpadE // C /=. (* b2i1 /= chunk_take_eq //.*)
rewrite -{3}(cat_take_drop (size bs %/ n * n)) -catA; congr.
 rewrite map_chunkK => //.
  rewrite size_take; first smt(size_ge0).
  rewrite {2}(divz_eq (size bs) n).
  case: (size bs %% n = 0)=> ?; first smt().
  rewrite ifT 1:/#.
  by rewrite dvdz_mull /#.
 by move=> l Hl; rewrite Hf 1:/# take_size_cat.
rewrite /flatten /= cats0 Hf; first smt(size_chunkremains).
rewrite take_cat ifF; first smt(size_chunkremains).
congr; first smt().
rewrite take_nseq size_chunkremains ler_minl; smt(size_ge0).
qed.
*).
...
proof.
rewrite /SSSs_to_TTTs size_flatten -map_comp /(\o) /=.
rewrite StdBigop.Bigint.sumzE.
rewrite StdBigop.Bigint.BIA.big_mapT /(\o) /=.
rewrite -(StdBigop.Bigint.BIA.eq_bigr _ (fun (x:SSS) => n)).
 by move=> i /= _; rewrite size_SSS_to_TTTs.
by rewrite StdBigop.Bigint.big_constz count_predT_eq.
qed.
...
lemma chunkfill_SSSs_from_TTTs k z l:
 chunkfill dSSS k (SSSs_from_TTTs (l ++ nseq z dTTT))
 = chunkfill dSSS k (SSSs_from_TTTs l).
proof.
admit.
qed.
...
move=> xs Hxs; rewrite /(\o) SSS_from_TTTsK //.
admitted (*
rewrite take_cat ifF 1:/# take_nseq.
smt(size_ge0).
qed.
*).
...
rewrite addzC mulzDr mulz1 nseq_add //; first 2 smt(n_gt0).
...
lemma bits2bytes2w64s l:
 w64L_from_bytes (bits2bytes l)
 = w64L_from_bits l.
admitted.

map w64_from_bytes (chunkify 8 (bits2bytes l))
= map (w64_from_bytes \o bits2bytes) (chunkify 64 l)
= map w64_from_bytes (map bits2bytes (chunkify 64 l))

ou seja:
chunkify k (bits2bytes l)
= map bits2bytes (chunkify (k*8) l)

mkseq (fun i => mkseq (fun k => ) sl)
= mkseq (
*)

(*
lemma nosmt size_ind ['a] (P : 'a list -> bool):
 (forall xs, (forall l, size l < size xs => P l) => P xs) => forall l, P l.
proof.
move=> H.
have Hind: forall (xs l:'a list), size l <= size xs => P l.
 elim => //=.
  smt.
 move=> xs Hxs l Hl.
 apply H => *. smt.
by move=> l; apply (Hind l).
qed.
*)
