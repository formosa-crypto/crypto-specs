...
proof. by elim: l1 => //= x xs IH; rewrite !w8L2bits_cons IH catA. qed.
...
proof.
by rewrite /w8L2bits /w2bits /(\bits8) /flatten /mkseq -iotaredE /=.
qed.
...
proof.
elim: l1 l2 => //=.
 move=> [|y ys]; rewrite w8L2bits_nil // w8L2bits_cons.
 by rewrite map2C /= /#.
move=> x xs IH1; elim => //=.
 rewrite map2C; first by move=> b1 b2; ring.
 by rewrite w8L2bits_cons map2C /= /#.
move=> y ys IH2.
...
proof.
elim: l1 => //=.
by move=> x xs IH; rewrite !w64L2bits_cons IH -!catA; congr.
qed.
...
proof.
elim: l1 l2 => //=.
 move=> [|y ys]; rewrite w64L2bits_nil // w64L2bits_cons.
 rewrite map2C /=.
  exact W64.xorwC.
 smt().
move=> x xs IH1; elim => //=.
 rewrite w64L2bits_nil map2C; first by move=> b1 b2; ring.
 rewrite w64L2bits_cons map2C /#.
move=> y ys IH2.
...
rewrite -drop_drop //.
move: l => [|x xs] //.
rewrite {1}(w8L2w64L_cons (x::xs) _) /=; first smt(size_ge0).
...
case: (0 <= i /\ i < (size l-1) %/ 8 + 1) => E; last first.
 ...
 by rewrite !W8u8.Pack.initiE //= unpack8E initiE //= get_zero.
...
proof.
elim: l => //= x xs IH.
by rewrite w64L2w8L_cons size_cat IH /#.
qed.
...
move=> x xs IH.
by rewrite /w64L2w8L map_cons flatten_cons w64L2bits_cons -IH w8L2bits_cat  w8L2bits_u64 //. 
qed.
...
by rewrite !w64L2w8L_cons -IH take_cat /= ifF /#.
qed.
