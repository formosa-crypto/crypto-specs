...
rewrite -!sem_decode1_corr /sem_decode1 /decode /= !get_of_list 1..8:/#.
do 8!(
...
rewrite to_uint_eq /= modz_small 1:/#.
...
have H : forall v idx, 0 <= v < 3 => 0 <= idx < 256 =>
     ...
     nth 0 (decode 12 (BytesToBits (to_list (subarray384 x v)))) (idx);
       last first.
rewrite (Array1024.get_of_list _ _ _ kb) /=.
...
have -> : 10240 = size (flatten (map W8.w2bits (to_list x))); 1:by smt(size_BytesToBits Array960.size_to_list).
rewrite take_size tP /= => i ib.
rewrite get_of_list 1:ib (nth_map (witness<:bool list>) 0);
  ...
  have /= [<- ]:= (Array1024.tP (Array1024.init (fun i => bs2int (nth witness (chunk 10 (flatten (map W8.w2bits (to_list x)))) i))) cc.`1); [| by apply ib | by smt(Array1024.initiE)].
rewrite /of_list -(Array1024.init_set witness) -JUtils.iotaredE /=.
...
+ move => i j v ib imod jval. congr.
  rewrite /sem_decode10_vec  to_uint_eq /=.
rewrite /decode_vec /BytesToBits !Array1024.get_of_list /=; 1:smt().
have -> : 10240 = size (flatten (map W8.w2bits (to_list x))); 1:by smt(size_BytesToBits Array960.size_to_list).
rewrite take_size /=.
...
+ move => i j k v ib imod kval jval. congr.
  rewrite /sem_decode10_vec  to_uint_eq /=.
rewrite /decode_vec /BytesToBits !Array1024.get_of_list /=; 1,2:smt().
have -> : 10240 = size (flatten (map W8.w2bits (to_list x))); 1:by smt(size_BytesToBits Array960.size_to_list).
rewrite take_size /=.
rewrite !(nth_map (witness<:bool list>) 0); 1,2:
...
have H0 : forall i j (v : W8.t Array960.t), 0 <= i < 1024 => i%%4 = 0 => j = i %/ 4 * 5 => 
  v.[j <- W8.of_int (sem_decode10_vec x).[i]] = v.[j <- x.[j]].
+ move => i j v ib imod jval. congr.
  rewrite /sem_decode10_vec  to_uint_eq /=.
rewrite /decode_vec /BytesToBits !Array1024.get_of_list /=; 1:smt().
have -> : 10240 = size (flatten (map W8.w2bits (to_list x))); 1:
     by smt(size_BytesToBits Array960.size_to_list).
rewrite take_size /=.
rewrite !(nth_map (witness<:bool list>) 0); 1:
by  rewrite !size_chunk /=;  by smt(size_BytesToBits Array960.size_to_list). 
...
+ do 192!(rewrite Hpos0 1..5:/#);
  ...
  have := Array1024.init_set witness (fun i => x.[i]).
  rewrite -JUtils.iotaredE /= => ->. 
  by rewrite tP => k kb; rewrite initiE /#.
